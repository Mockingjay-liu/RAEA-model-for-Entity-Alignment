...
def set_random_seed(seed_value=0, print_info=False):
    ...
    torch.manual_seed(seed_value)  # cpu  vars  固定CPU生成随机数的种子，使得每次生成的随机数相同
    np.random.seed(seed_value)  # cpu vars
    if torch.cuda.is_available():
        torch.cuda.manual_seed_all(seed_value)  # 为所有的GPU设置种子
    random.seed(seed_value)


def get_hits(sim, top_k=(1, 10), channel_result=False, print_info=True, device='cpu'):
    ...
    channel_hits_1 = (top_lr[0] + top_rl[0]) / 2
...
def topk(sim, top_k=(1, 10, 50, 100), device='cpu'):
    ...
    def _opti_topk_batched(sim):
        ...
        for i in range(0, test_num, batch_size):
            ...
            sorted_arg = torch.argsort(batch_sim)  # 沿着最低维度升序排列，返回升序排列后最低维度的下标， (1024, 9000)
            true_pos = torch.arange(batch_sim.shape[0]).reshape((-1, 1)).to(device) + i   # (1024, 1) #
            locate = sorted_arg - true_pos  # 对sorted_arg每一列减去true_pos对应位置的值
            del sorted_arg, true_pos
            locate = torch.nonzero(locate == 0,)  # 返回一个包含输入locate中元素值为0的索引的张量.输出张量中的每行包含locate中零元素的索引.
            cols = locate[:, 1]  # Cols are ranks
            cols = cols.float()
