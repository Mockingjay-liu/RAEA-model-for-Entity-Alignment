...
class GCN(nn.Module):
    ...
    def forward(self, x, edge_index):
        ...
        deg = degree(edge_index_i, x.size(0), dtype=x.dtype)  # KG中每个entity的degree
        deg_inv_sqrt = deg.pow(-0.5)
        norm = deg_inv_sqrt[edge_index_j] * deg_inv_sqrt[edge_index_i]
        x = F.relu(spmm(edge_index[[1, 0]], norm, x.size(0), x.size(0), x))  # edge_index[[1, 0]]表示KG稀疏矩阵中数值的index, 对应于norm中的值, x是表示entity embedding的dense matrix
        return x
...
class Highway(nn.Module):
    ...
    def forward(self, x1, x2):  # x1经过gcn得到x2, x2是x1的下一层
...
class GAT_E_to_R(nn.Module):  # 由 entity embedding 生成 relation representation
...
class GAT_R_to_E(nn.Module):
    ...
    def forward(self, x_e, x_r, edge_index, rel):
        ...
        x_e_h = spmm(torch.cat([edge_index_h.view(1, -1), rel.view(1, -1)], dim=0), alpha, x_e.size(0), x_r.size(0), x_r)  # entity的out-relation embedding, 每个entity的维度为r-hidden
        alpha = softmax(F.leaky_relu(e_t + e_r).float(), edge_index_t)
        x_e_t = spmm(torch.cat([edge_index_t.view(1, -1), rel.view(1, -1)], dim=0), alpha, x_e.size(0), x_r.size(0), x_r)  # entity的in-relation embedding, 每个entity的维度为r-hidden
...
class RAGA(nn.Module):  # without RGAT
    def __init__(self, e_hidden=128, r_hidden=64):  # 默认输入RAGA的ent emb是128维，输出的rel emb是64维
        super(RAGA, self).__init__()
        self.gcn1 = GCN(e_hidden)
        self.highway1 = Highway(e_hidden)
        self.gcn2 = GCN(e_hidden)
    ...
    def forward(self, x_e, edge_index, rel, edge_index_all, rel_all): # 输入一张KG的ent emb, 单向的rel triples index, 双向的rel triples index, 单向的relation编号, 双向的relation编号
        # Highway-GCN的深度是2
        x_e = self.highway1(x_e, self.gcn1(x_e, edge_index_all))
        x_e = self.highway2(x_e, self.gcn2(x_e, edge_index_all))
        return x_e  # 输出的ent维度是 128维 (只能跑这个维度的数据！！)
"""
...
class GraphConvolution(nn.Module):
    def __init__(self, input_dim, output_dim, featureless=False, act_func=F.relu, residual=True):
        ...
        # literal和digital通道进入两个if代码块， structure和name通道不进入两个if代码块
    ...
    def forward(self, adj, feats): # literal和digital通道进入两个if代码块， structure和name通道不进入两个if代码块
        # 这个forward函数输入KG邻接矩阵和ent emb(每个entity的维度是128，因为attribute的编码维度也是128)，是在entity之间用mean aggregator传播节点特征，只考虑了结构，没考虑到关系的差异
        to_feats = torch.sparse.mm(adj, feats)
        degree = torch.sparse.sum(adj, dim=1).to_dense().reshape(-1, 1)
        to_feats = to_feats / degree # 这一步是mean aggregator中mean的操作
        if not self.featureless:
            to_feats = self.linear(to_feats)
        to_feats = self.act_func(to_feats) # 至此完成mean aggregator的计算，得到feats下一层的to_feats
        if self.residual:
            if feats.shape[-1] != to_feats.shape[-1]:
                to_feats = self.root(feats) + to_feats
            else:
                to_feats = feats + to_feats # 得到feats与下一层的to_feats相加得到最终的下一层feats
        return to_feats
...
class AttributedEncoder(nn.Module):  # 只在literal和digital中会用到
    ...
    def forward(self, attribute_triples, att_feats, val_feats, ent_feats): # 使用attention机制为每个entity聚合attribute，返回聚合后的entity embedding
        ...
        N = ent_feats.shape[0]  # 一张图的 entity 的数量
        E = attribute_triples.shape[0]  # 一张图的 attribute triple 的数量
...
class AttSeq(nn.Module):
    def __init__(self, layer_num, sr_ent_num, tg_ent_num, dim, drop_out, att_num, attribute_triples_sr, attribute_triples_tg, value_embedding, edges_sr, edges_tg, residual=True):
        ...
        self.residual = residual
        ## KG Feature Loading
        self.edges_index_sr = edges_sr.t()  # edges_sr是tensor类型，self.edges_sr是Parameter类型
