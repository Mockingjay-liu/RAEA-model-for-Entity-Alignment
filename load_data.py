...
class BERT(object): # 使用MP-net
    ...
    def __init__(self):
        ...
        self.model_save_path = '../../SimCSE_multilin/output/training_simcse-sentence-transformers/paraphrase-multilingual-mpnet-base-v2-2022-07-26_12-42-26'
    ...
    def pooled_encode_batched(self, sentences, batch_size=512, layer=1, save_gpu_memory=False):
        ...
        outputs = []
        for batch in tqdm(sent_batch):  # 这一步如果用CPU需要5小时
            #out = self.pooled_bert_encode(batch, layer)
            out = self.model.encode(batch, convert_to_tensor=True)
            if save_gpu_memory:
                out = out.cpu()
            outputs.append(out)
...
#         sentences = torch.tensor([sent + (max_len - len(sent)) * [self.pad_token_id] for sent in sentences]).to(self.device)
#         with torch.no_grad():
...
def read_file(path, parse_func):
    ...
    with open(path, 'r', encoding='utf8') as f:
        ...
        if line.isdigit():
            ...
            f.seek(0)
        lines = f.readlines()
    if 'id2att' in str(path):
        print()
    lines = parse_func(lines)

    if len(lines) != num and num >= 0:
        print_time_info('File: %s has corruptted, data_num: %d/%d.' % (path, num, len(lines)))
        raise ValueError()
    return lines
...
def _load_seeds(directory, train_seeds_ratio):
    ...
    if train_data_path.exists() and valid_data_path.exists() and test_data_path.exists():
        ...
        if not test_sr_ids_path.exists():
            ...
            test_entity_seeds = valid_entity_seeds
    return train_entity_seeds, valid_entity_seeds, test_entity_seeds, entity_seeds # 都是含有表示seed entity pairs的元组的list
...
def _load_dbpedia_properties(data_path, entity2id, language, filter_alias=False): # 返回一个list，包含属性三元组（ent_id, value, attribute）
    ...
    for line in lines:
        try:
            subject, property, value, _ = line  # 获取entity，属性，属性值
        except ValueError:
            subject, property, value = line
        subject = remove_prefix(subject)  # 去除uri的前缀，得到entity name
...
def _get_train_value_and_attribute(train_ent_ids, att_triples):  # 返回训练集entity对应的属性及属性值(value, att)
...
def _split_digit_attribute_and_literal_attribute(value_and_attribute_pairs, digit_threshold, att_set):  # 输入source和target训练集所有的（属性值，属性）、数值阈值、att2id对应的attribute集合
    numeral = Numeral()
    att_is_number = {}  # {属性：[数值取值数量，文本取值数量]} 同个属性可能既出现数值型取值，又出现文本型取值
    for value, att_id in value_and_attribute_pairs:
        # 0 digit, 1 literal
        is_numeral, number = numeral.is_numeral(value)
        if att_id in att_is_number:
            if is_numeral:  # 如果是数值型，就
    ...
    digit_atts = {att for att, count in att_is_number.items() if count[0] / sum(count) > digit_threshold}  # 如果某个attr的取值中，数值型取值的占比大于digit_threshold，就将该属性归类到digit_atts集合中，否则归类到literal_atts
    ...
    return digit_att2id, literal_att2id  # 返回两个字典分别是{数值型属性: id}、{文本型属性: id}


def _split_digit_and_literal_triple(att_triples, digit_att2id, literal_att2id):  # 将att_triples划分为digital和literal，并返回
    digit_triples = []
    literal_triples = []
    digit_num = 0
    literal_num = 0
...
class ValueEmbedding(object):
    ...
    def encode_value(self, value_seqs):
        ...
        best_layer = 1
        value_embedding = self.bert.pooled_encode_batched(id2value, layer=best_layer, batch_size=128, save_gpu_memory=True) # 这一行很费时间！！！
...
class LoadData(object):
    def __init__(self, train_seeds_ratio, directory, nega_sample_num, name_channel, attribute_channel, digit_literal_channel, device='cpu'):
        self.device = device
        ...
        self.load_seed_alignment()  # 加载seed alignment，划分为train/valid/test，并分开每一对entity的source entity和target entity
        self.load_structure_feature()  # 加载图谱结构特征，也就是 source和target graph的relation triples以及att2id
        if name_channel:
            self.load_name_feature('name_embedding_%s.npy' % self.language_sr, 'name_embedding_%s.npy' % self.language_tg)
        if attribute_channel or digit_literal_channel:
            self.load_attribute_feature(attribute_channel, digit_literal_channel)  # 加载图谱属性特征，也就是 source和target graph的attribute triples以及embedding
        self.negative_sample()
        self.to_torch() # 将验证集转为tensor
    ...
    def update_negative_sample(self, sr_nega_sample, tg_nega_sample):
        ...
        self.train_sr_ent_seeds = torch.cat((self.sr_posi_sample, sr_nega_sample), dim=1)  # 训练集中source graph对应的entity生成的负样本
        self.train_tg_ent_seeds = torch.cat((self.tg_posi_sample, tg_nega_sample), dim=1)  # 训练集中target graph对应的entity生成的负样本

    def negative_sample(self):  # 生成负样本
    ...
    def load_structure_feature(self):
        ...
        id2atts = read_mapping(self.directory / 'id2atts.txt')  # dict, {编号: attribute的取值}
        self.att2id = {att: idx for idx, att in id2atts.items()}  # dict, {attribute的取值: 编号}
        self.att_num = len(self.att2id)
        self.triples_sr, self.id2entity_sr, self.id2relation_sr = _load_language(self.directory, self.language_sr)  # 返回source graph的triples（s,o,p），以及各种id对应的entity和relation名称
        self.triples_tg, self.id2entity_tg, self.id2relation_tg = _load_language(self.directory, self.language_tg)  # 返回target graph的triples（s,o,p），以及各种id对应的entity和relation名称
        self.sr_ent_num = len(self.id2entity_sr)
        self.tg_ent_num = len(self.id2entity_tg)

    def load_name_feature(self, value_embedding_cache_path_sr,value_embedding_cache_path_tg):
        temp_file_dir = self.directory / 'running_temp'
        value_embedding_cache_path_sr = temp_file_dir / value_embedding_cache_path_sr
        value_embedding_cache_path_tg = temp_file_dir / value_embedding_cache_path_tg
        # Load translations
        if value_embedding_cache_path_sr.exists() and value_embedding_cache_path_tg.exists():
            self.sr_embed = nn.Parameter(torch.from_numpy(np.load(value_embedding_cache_path_sr)))
            self.tg_embed = nn.Parameter(torch.from_numpy(np.load(value_embedding_cache_path_tg)))
            print_time_info("Loaded name embedding from %s and %s." % (value_embedding_cache_path_sr, value_embedding_cache_path_tg))
        else:
            ...
            np.save(value_embedding_cache_path_sr, self.sr_embed.cpu().numpy())
            np.save(value_embedding_cache_path_tg, self.tg_embed.cpu().numpy())
            del bert

    def load_seed_alignment(self):
        # Load alignment seeds
        train_entity_seeds, valid_entity_seeds, test_entity_seeds, entity_seeds = _load_seeds(self.directory, self.train_seeds_ratio)
        ...
        self.train_sr_ent_seeds_ori = np.asarray(train_sr_ent_seeds_ori)  # 训练集中source graph对应的entity
        self.train_tg_ent_seeds_ori = np.asarray(train_tg_ent_seeds_ori)  # 训练集中target graph对应的entity
    ...
    def load_attribute_feature(self, load_attribute, load_digit_literal):
        ...
        att_triples_sr = _load_dbpedia_properties(directory / ("atts_properties_%s.txt" % language_sr), entity2id_sr, language_sr)
        att_triples_tg = _load_dbpedia_properties(directory / ("atts_properties_%s.txt" % language_tg), entity2id_tg, language_tg)
        ...
        if load_attribute:
            ...
            self.value_embedding, self.id2value = value_embed_encoder.load_value(value_seq_sr + value_seq_tg, value_embed_cache_path, id2value_cache_path, )
            value2id = {value: idx for idx, value in enumerate(self.id2value)}
            value_id_seq_sr = [[value2id.get(value, value2id['[PAD]']) for value in value_seq] for value_seq in value_seq_sr]
            value_id_seq_tg = [[value2id.get(value, value2id['[PAD]']) for value in value_seq] for value_seq in value_seq_tg]
        ...
        if load_digit_literal:
            ...
            digit_threshold = 0.5
            digit_att2id, literal_att2id = _split_digit_attribute_and_literal_attribute(  # 返回两个字典分别是{数值型属性: id}、{文本型属性: id}
            ...
            digit_triples_sr, literal_triples_sr = _split_digit_and_literal_triple(att_triples_sr, digit_att2id, literal_att2id)
            digit_triples_tg, literal_triples_tg = _split_digit_and_literal_triple(att_triples_tg, digit_att2id, literal_att2id)

            digit_ent_id_seq_sr, digit_att_id_seq_sr, digit_value_seq_sr = transform_triple2seq(digit_triples_sr, language_sr, False)
            digit_ent_id_seq_tg, digit_att_id_seq_tg, digit_value_seq_tg = transform_triple2seq(digit_triples_tg, language_tg, False)

            literal_ent_id_seq_sr, literal_att_id_seq_sr, literal_value_seq_sr = transform_triple2seq(literal_triples_sr, language_sr, False)
            literal_ent_id_seq_tg, literal_att_id_seq_tg, literal_value_seq_tg = transform_triple2seq(literal_triples_tg, language_tg, False)
            ...
            self.literal_value_embedding, self.literal_id2value = value_embed_encoder.load_value(
                literal_value_seq_sr + literal_value_seq_tg, literal_value_embed_cache_path, literal_id2value_cache_path)
...
def transform_triple2seq(att_triples, language, concate_values=False):
    ...
    if language in {'zh', 'en', 'ja', 'fr'}:
        ...
        top_k_att = 3
    ent_id_seq = []  # ent_id集合
    prop2value_seq = []  # 列表，包含每个entity对应的所有属性及属性值，每个entity对应一个字典{att_id: value}，
    for ent_id, value, att_id in att_triples:
        ...
        if not concate_values:
            ...
            if att_id in prop2value_seq[-1]:
                ...
                prop2value_seq[-1][att_id] = value
    att_id_seq = []  # 包含所有的entity对应的att_id集合
    value_seq = []  # 包含所有的entity对应的att的属性集合
    ...
    return ent_id_seq, att_id_seq, value_seq  # ent_id， 每个entity的att_id集合， 每个entity的att的values集合
