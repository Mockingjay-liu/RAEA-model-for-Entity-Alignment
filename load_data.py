...
class BERT(object): # use MP-net for training
...
class ValueEmbedding(object):
    ...
    def encode_value(self, value_seqs):
        ...
        best_layer = 1
        value_embedding = self.bert.pooled_encode_batched(id2value, layer=best_layer, batch_size=128, save_gpu_memory=True)
...
class LoadData(object):
    ...
    def update_negative_sample(self, sr_nega_sample, tg_nega_sample):
        ...
        self.train_sr_ent_seeds = torch.cat((self.sr_posi_sample, sr_nega_sample), dim=1)  # Negative samples generated by entity corresponding to source graph in training set
        self.train_tg_ent_seeds = torch.cat((self.tg_posi_sample, tg_nega_sample), dim=1)  # Negative samples generated by entity corresponding to target graph in training set

    def negative_sample(self):  # Negative sample generation
    ...
    def load_structure_feature(self):
        ...
        self.triples_sr, self.id2entity_sr, self.id2relation_sr = _load_language(self.directory, self.language_sr)  # Returns the triples (s,o,p) of the source graph and the entity and relation names corresponding to the various ids
        self.triples_tg, self.id2entity_tg, self.id2relation_tg = _load_language(self.directory, self.language_tg)  # Returns the triples (s,o,p) of the target graph and the entity and relation names corresponding to the various ids
    ...
    def load_seed_alignment(self):
        ...
        self.train_sr_ent_seeds_ori = np.asarray(train_sr_ent_seeds_ori)  # Train the entity corresponding to source graph in the set
        self.train_tg_ent_seeds_ori = np.asarray(train_tg_ent_seeds_ori)  # The entity corresponding to the target graph in the training set
