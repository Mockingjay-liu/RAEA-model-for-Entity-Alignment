...
class AlignLoss(nn.Module):
    ...
    def forward(self, repre_sr, repre_tg):
        ...
        loss = self.criterion(torch.cat((sr_true, tg_true), dim=0),  # 正样本(e1, e2)的e1集合与e2集合相接，e1来自source，e2来自target，长度 = 正样本数量 * 每个正样本的负采样数量 * 2
                              torch.cat((tg_true, sr_true), dim=0),  # 正样本(e1, e2)的e2集合与e1集合相接，e1来自source，e2来自target，长度 = 正样本数量 * 每个正样本的负采样数量 * 2
                              torch.cat((tg_nega, sr_nega), dim=0))  # 负样本(e2_, e1_)的e2_集合与e1_集合相接, e1_来自source，e2_来自target，长度 = 正样本数量 * 每个正样本的负采样数量 * 2
        return loss
...
if __name__ == '__main__':
    '''
    python train_subgraph.py --dataset DBP15k/zh_en --gpu_id 0 --nega_sample_num 25
    '''
    import argparse
    ...
    parser.add_argument('--nega_sample_num', type=int, required=True)  # For DBP15k, sugguest nega_sample_num = 25. For DBP100k, sugguest nega_sample_num = 5
